...
IN_FILE_PATH = "steam_data/app_data.csv"
...
# 외부에 출력할 파일 이름
OUT_FILE_PATH = "steam_data/" + QRY_REVIEW_TYPE + "_reviews.csv"
...
def preprocess_review(data):
    ...
    # steam_data[REVIEW] = re.sub(r'[^a-zA-Z0-9]', ' ', steam_data[REVIEW])
    data[REVIEW] = re.sub(r'[^가-힣]', ' ', data[REVIEW])

    # 모든 알파벳을 소문자로 치환하고 양 쪽 끝의 공백을 제거
    # steam_data[REVIEW] = steam_data[REVIEW].lower().strip()
